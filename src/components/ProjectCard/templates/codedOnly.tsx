...
export default function ProjectCodedOnly({ title, className, src, type, github }: Omit<ProjectCardDTO, 'summary' | 'link'>) {
    return (
        <ProjectCard.Main className="col-span-12 md:col-span-6 4xl:col-span-6 flex-col px-0 overflow-hidden gap-1">
            <ProjectCard.Tumble src={src} link={github} title={title} className="w-full h-[65%]" />
            <ProjectCard.Aside className="w-full h-[35%] flex flex-col items-start justify-between px-8 pb-8">
                <ProjectCard.Header >
